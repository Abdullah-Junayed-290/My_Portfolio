    ...
    <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-12">
      ...
      <header className="mb-8">
        ...
        <p className="mt-3 max-w-3xl text-slate-600 dark:text-slate-300 text-lg">
          I’m <span className="font-semibold text-indigo-600 dark:text-indigo-400">Md. Abdullah Junayed</span>,
          a budding full‑stack web developer from Bangladesh. I’m focused on modern
          JavaScript, React, and backend fundamentals — building real projects and
          leveling up every week.
        </p>
        <p className="mt-2 text-slate-600 dark:text-slate-300">
          আমি আব্দুল্লাহ জুনায়েদ — শিখছি, বানাচ্ছি, আর উন্নত করছি। লক্ষ্য: দক্ষ ফুল‑স্ট্যাক
          ডেভেলপার হওয়া, প্র্যাকটিক্যাল প্রোজেক্ট দিয়ে ক্যারিয়ার গড়া।
        </p>
      </header>

      {/* Quick facts */}
      <section className="mb-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <FactCard label="Current focus" value="React • Node.js • Tailwind • APIs" />
        <FactCard label="Open to" value="Internships • Freelance • Collaborations" />
      ...
      <section className="mb-12">
        ...
        <ol className="mt-4 space-y-4">
          {milestones.map((m, i) => (
            <li key={m.title} className="relative rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
      ...
      <section className="mb-12">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {values.map(({ k, v }) => (
            ...
            >
              <h3 className="font-semibold text-slate-900 dark:text-white">{k}</h3>
              <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{v}</p>
            </div>
          ))}
        </div>
      </section>
